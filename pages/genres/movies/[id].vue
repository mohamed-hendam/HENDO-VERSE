<template>
  <div class="genres-movies-selected">
    <h1 class="h1-genres-movies-selected">
      Here Is Your {{ selectedGenre }} Movies Collection
    </h1>
    <p>
    ...
    <v-container>
      <v-row>
        <v-col
          ...
          :key="movie.id"
        >
          <v-card class="card-genres-movies-selected">
            <nuxt-link
              :to="`/details/movieDetails/${movie.id}`"
              class="card-item-genres-movies-selected"
            >
              <img
...
const fetchShowData = async (id, traget) => {
  const { data } = await useFetch(
...
await fetchGenres();
await fetchShowData(genreId, movieData);
